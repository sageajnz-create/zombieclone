...
{
    /// <summary>
    /// Server-side weapon resolution. Fire rate, ammo, reload and hitscan tracing all
    ...
    public sealed class WeaponRuntime : MonoBehaviour
    {
        ...
        public void ServerInitialise(WeaponDefinition definition, StatBlock stats, PlayerId owner, RunContext run)
        {
            ...
            _magazine = Mathf.RoundToInt(_stats.ResolveFor(_definition.MagazineSize, StatId.MagazineSize, _definition.Tags));
        ...
        public void ServerTick(InputFrame frame, Transform aimOrigin, float now)
        {
        ...
        private void Fire(Transform aimOrigin, float now)
        {
            ...
            {
                Vector3 direction = ApplySpread(aimOrigin.forward, spread);
        ...
        private Vector3 ApplySpread(Vector3 forward, float degrees)
        {
            ...
            // Uniform disc sample so pellets do not cluster toward the centre.
